...
const getShouldSell = async (req, res, next) => {
    ...
    if (!!purchasedCryptos && !!purchasedCryptos.length) {
        for (const item of purchasedCryptos) {
            ...
            console.log(item);
            const percentageDiff = ((foundItems.price * TRANSACTION_FEE) / (item.price * TRANSACTION_FEE)) * 100;
            ...
            data.push({
                percentageDiff, ...item?._doc || {}, first, second, third,
                currentPrice: foundItems.price,
                priceBoughtFor: item.price,
                potentialProfit: (foundItems.price * TRANSACTION_FEE) - (item.price * TRANSACTION_FEE),
            });
        }
    }
